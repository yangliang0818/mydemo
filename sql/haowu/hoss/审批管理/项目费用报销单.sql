...
SELECT
            ...
            instance.start_user_name AS startUserName,
            instance.start_time      AS startTime
            FROM cm_project_fee fee join cm_project_fee_info feeInfo on fee.id = feeInfo.fee_id JOIN wf_instance instance ON fee.wf_instance_id = instance.id
            WHERE instance.status IN (2) AND fee.project_id = 1003743741
            AND NOT exists(SELECT contractFeeInfo.fee_info_id FROM cm_apply_contract contract,cm_apply_contract_fee_info contractFeeInfo,wf_instance wf
                WHERE wf.id = contract.wf_instance_id AND contractFeeInfo.apply_contract_id = contract.id AND contract.fee_id = fee.id and contractFeeInfo.fee_info_id=feeInfo.id AND wf.STATUS IN (1, 2, 5, 6))
            AND NOT exists(SELECT repayInfo.fee_info_id FROM cm_project_repay repay,cm_project_repay_info repayInfo,wf_instance wf
                WHERE repay.id = repayInfo.repay_id AND repay.wf_instance_id = wf.id AND repay.fee_id = fee.id and repayInfo.fee_info_id=feeInfo.id AND wf.STATUS IN (1, 2, 5, 6))
            group by fee.id;
